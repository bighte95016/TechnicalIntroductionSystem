...
from logging_util import log_question_answer  # 新增
...
def create_qa_chain(llm, vectorstore):
    ...
    try:
        ...
        # 定義 Prompt 模板 (極簡版本)
        template = """
        你是一個介紹全景抬頭式顯示器(P-HUD)的工研院專家，請根據以下提供的技術資訊資訊，簡潔地回答問題。
        使用者的問題皆圍繞在P-HUD相關，
...
# --- 重構出來的處理單一問題的函數 ---
def process_question(question: str, qa_chain, logger_func):
    """
    
    處理單個問題，調用 LLM chain 並記錄結果。

    Args:
        question (str): 使用者輸入的問題。
        chain: 用於調用 LLM 的 chain 物件 (例如 qa_chain)。
        logger_func: 用於記錄問題和答案的函數 (例如 log_Youtube)。
    ...
    print("\n答案：")
    print(answer)
    log_question_answer(question, answer) # 新增
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            """ 此區塊已被重購至 process_question(question: str, chain, logger_func)
            ...
            """
            answer, source_docs = process_question(question, qa_chain, log_question_answer)
