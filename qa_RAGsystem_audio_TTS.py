...
# --- TTS引擎初始化 ---
tts_engine = None # 全局變量，用於存儲TTS引擎
...
def initialize_tts_engine():
    ...
    if tts_engine is None:
        ...
        try:
            ...
            # 設置TTS引擎屬性
            tts_engine.setProperty('rate', TTS_RATE)  # 語速
            tts_engine.setProperty('volume', TTS_VOLUME)  # 音量
            
            # 獲取可用的語音
            voices = tts_engine.getProperty('voices')
            print(f"已檢測到 {len(voices)} 個語音")
            
            # 輸出可用語音信息（可選）
            for i, voice in enumerate(voices):
                print(f"語音 #{i}: {voice.name} ({voice.id}) - 語言: {voice.languages}")
            
            print("TTS 引擎初始化成功。")
            return True
        except Exception as e:
            print(f"初始化 TTS 引擎時發生錯誤: {e}")
            print("語音回覆功能將不可用。")
            return False
...
def text_to_speech(text, language):
    ...
    try:
        ...
        if language == "zh":
            # 嘗試找到中文語音
            chinese_voices = [v for v in voices if any(lang.startswith('zh') for lang in v.languages)]
            if chinese_voices:
                tts_engine.setProperty('voice', chinese_voices[0].id)
            # 如果沒有中文語音，使用默認語音
        else:  # 英文或其他語言
            # 嘗試找到英文語音
            english_voices = [v for v in voices if any(lang.startswith('en') for lang in v.languages)]
            if english_voices:
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            # 使用 TTS 朗讀答案
            text_to_speech(answer, answer_lang)
