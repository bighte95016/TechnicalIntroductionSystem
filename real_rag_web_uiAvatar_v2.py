...
<head>
    ...
    <style>
        ...
        .main-panel {
            ...
            background: white;
            border-radius: 20px;
            padding: 0; /* No padding, handled by overlay */
            box-shadow: 0 20px 40px rgba(0,0,0,0.1);
            display: flex;
            flex-direction: column;
            align-items: center;
            overflow: hidden; /* Hide overflow, manage scrolling in children */
            position: relative; /* For absolute positioning of children */
            transition: width 0.2s ease, height 0.2s ease; /* For smooth resizing */
        }
        ...
        .avatar-container {
            ...
            z-index: 1;
            overflow: hidden;
            /* Flexbox properties removed for a more robust centering method */
        }
        ...
        .status.ready, .status.loading {
            /* These states will now use the new base .status style */
        }
        
        .status.error {
            /* Error state keeps its distinct style, overrides base */
            background: #f8d7da;
            color: #721c24;
            border-color: #f5c6cb; /* Only need to override border-color */
        }
        ...
        .recording-indicator {
            text-align: center;
            margin-bottom: 10px; /* Swapped from margin-top */
            font-size: 14px;
            color: #666;
            height: 20px;
            width: 100%;
        ...
        .chat-history {
            ...
            margin-bottom: 1px; /* Reduced space below chat */
            padding: 15px;
            border-radius: 15px;
            background: transparent; /* Removed the semi-transparent background */
            pointer-events: auto;
        ...
        .message {
            /* margin-bottom is now on the message-row */
            padding: 15px;
            border-radius: 10px;
        }
        ...
        .question-msg {
            background: rgba(227, 242, 253, 0.7); /* Semi-transparent light blue */
            /* border-left removed */
            color: #1a252f;
            text-shadow: 0px 0px 2px rgba(255,255,255,0.5); /* Subtle shadow for readability */
            width: 66.67%;
            box-sizing: border-box;
        }
        
        .answer-msg {
            background: rgba(243, 229, 245, 0.7); /* Semi-transparent light purple */
            /* border-left removed */
            color: #2c1d30;
            text-shadow: 0px 0px 2px rgba(255,255,255,0.5); /* Subtle shadow for readability */
            width: 66.67%;
        ...
        @media (max-width: 768px) {
            ...
            #question {
                width: 100%;
                order: 1; /* Make textarea first on wrap */
                margin-top: 10px;
            }
            .voice-button {
                order: 2;
        ...
        .content-overlay {
            ...
            flex-direction: column;
            align-items: center;
            /* justify-content: flex-end; Removed to allow top/bottom separation */
...
<body>
        ...
        <div class="main-panel">
            ...
            <div class="content-overlay">
                <div id="status" class="status loading">
                    <span id="statusText">AI助理準備中</span>
                </div>
                ...
                <div class="input-container">
                    ...
                    <div class="input-area">
                        ...
                        <textarea id="question" placeholder="請在此輸入問題..."></textarea>
                        <button class="button" id="textBtn" onclick="askTextQuestion()">發送</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        socket.on('show_question', function(data) {
            console.log('🎤 收到Whisper識別結果:', data.question);
            addQuestionToChat(data.question);
        });

        socket.on('show_final_answer', function(data) {
            console.log('✅ 收到最終回答');
            addAnswerToChat(data.answer);
        });

        socket.on('answer_error', function(data) {
            console.error('❌ 回答生成時發生錯誤:', data.message);
            addAnswerToChat(`錯誤: ${data.message}`);
        });
        ...
        function addMessage(question, answer, audioUrl = null, language = 'zh') {
            ...
            const messageHTML = `
                ...
                <div class="message-row answer-row">
                    <div class="message answer-msg">
                        <strong>回答：</strong><span class="answer-text">${answer.trim()}</span>
                        <div class="timestamp">${timestamp}</div>
                    </div>
                </div>
            `;
        ...
        function addAnswerToChat(answer) {
            ...
            `;
            chatHistory.innerHTML += answerHTML;
            chatHistory.scrollTop = chatHistory.scrollHeight;
        }
        ...
        async function askTextQuestion() {
            ...
            try {
                ...
                addAnswerToChat('處理問題時發生錯誤: ' + error.message);
                alert('處理問題時發生錯誤: ' + error.message);
            } finally {
                textBtn.disabled = false;
                textBtn.textContent = '發送';
            }
        }
        ...
        async function processVoiceRequest(audioBlob) {
            try {
                ...
                if (!response.ok) {
                    ...
                    const message = data.message || `HTTP 錯誤: ${response.status}`;
                    console.error("語音請求失敗:", message);
                    // Rely on backend to emit error via websocket
                }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function resizePanel() {
                ...
                if (!aspectRatio || isNaN(aspectRatio)) {
                    console.log('無法獲取虛擬人物的寬高比，跳過面板尺寸調整。');
                    return;
                }
                ...
                // 比較可用空間與圖片的寬高比，以決定尺寸限制因素
                if ((availableWidth / availableHeight) > aspectRatio) {
                    // 可用空間比圖片更寬，因此高度是限制因素
                    newHeight = availableHeight;
                    newWidth = availableHeight * aspectRatio;
                } else {
                    // 可用空間比圖片更窄（或等寬），因此寬度是限制因素
            ...
            if (avatarImage.complete) {
                ...
                avatarImage.onload = resizePanel;
            }
...
def initialize_rag_system():
    ...
    try:
        ...
        print("✅ RAG系統初始化完成！")
        socketio.emit('system_status', {'status': 'ready', 'message': 'AI助理已就緒！可以開始提問了'})
...
def voice_question():
    ...
    try:
        if 'audio' not in request.files:
            return jsonify({'success': False, 'message': '未找到音頻文件'}), 400
        ...
        try:
            ...
            if not question or len(question.strip()) == 0:
                socketio.emit('answer_error', {'message': '無法識別語音內容'})
            ...
            socketio.emit('show_final_answer', {'answer': answer})
            return jsonify({'success': True})
            
        except Exception as inner_error:
            print(f"處理音頻文件時發生內部錯誤: {inner_error}")
            socketio.emit('answer_error', {'message': str(inner_error)})
            return jsonify({'success': True}) # Return True to not trigger fetch error
            
    except Exception as e:
        error_msg = f'處理語音時發生錯誤: {str(e)}'
        print(f"❌ {error_msg}")
        socketio.emit('answer_error', {'message': error_msg})
...
def handle_connect():
    ...
    print('客戶端已連接')
    if system_ready:
        emit('system_status', {'status': 'ready', 'message': 'AI助理已就緒！可以開始提問了。'})
    else:
