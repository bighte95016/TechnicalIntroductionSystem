...
import sounddevice as sd           # 改用 sounddevice 播放音頻 (新增)
import soundfile as sf             # 音頻文件處理 (新增)
import tempfile
...
TTS_SAMPLE_RATE = 24000  # 音頻采樣率 (XTTS v2 標準採樣率)
...
# 音頻緩存字典 {(text, language, temperature): audio_data}
TTS_CACHE = {}  # 音頻緩存
...
def initialize_coqui_tts():
    """載入 Coqui TTS 模型並進行必要的預熱（聲音克隆版本）"""
    global coqui_tts_model
    if coqui_tts_model is None:
        ...
        try:
            ...
            if not os.path.exists(SPEAKER_WAV_FILE):
                print(f"警告：說話人音頻文件 '{SPEAKER_WAV_FILE}' 不存在！")
                print("請確保有相應的音頻文件用於聲音克隆。")
                return False
            else:
            ...
            print("Coqui TTS 引擎初始化成功（聲音克隆模式）。")
            return True
        except Exception as e:
...
def text_to_speech(text, language="zh"):
    """使用 Coqui TTS 生成語音數據並播放（聲音克隆版本）
    
    參數:
        text (str): 要播放的文字內容
        language (str): 語言代碼，'zh' 表示中文，'en' 表示英文
    """
    global coqui_tts_model, TTS_CACHE
    ...
    # 檢查說話人音頻文件
    if not os.path.exists(SPEAKER_WAV_FILE):
        print(f"❌ 說話人音頻文件 '{SPEAKER_WAV_FILE}' 不存在！")
        return False

    # 映射語言代碼 (XTTS v2 使用不同的語言代碼)
    coqui_lang = 'zh' if language == 'zh' else 'en'
    
    # 生成緩存鍵
    cache_key = (text, coqui_lang, TTS_TEMPERATURE)
    
    print(f"🔊 使用 Coqui TTS 聲音克隆 ({coqui_lang}, 溫度: {TTS_TEMPERATURE}) 播放: {text[:30]}...")
    ...
    with playback_lock:
        try:
            ...
            try:
                print("🎵 正在播放...")
                # sounddevice 直接支持浮點數音頻數據
                sd.play(wav, samplerate=TTS_SAMPLE_RATE, blocking=True)
                print("✅ 播放完成")
                return True
                
            except Exception as sd_error:
                print(f"❌ sounddevice 播放時發生錯誤: {sd_error}")
                return False
    ...
    def play_prompt_in_background():
        ...
        with prompt_lock:
            try:
                ...
                # 生成緩存鍵
                cache_key = (prompt_text, prompt_lang, TTS_TEMPERATURE)
